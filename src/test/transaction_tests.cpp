...
BOOST_AUTO_TEST_CASE(test_basic_joinsplit_verification)
{
    ...
    {
        JSDescription jsdesc(*pzelcashParams, pubKeyHash, rt, inputs, outputs, 0, 0);
        BOOST_CHECK(jsdesc.Verify(*pzelcashParams, verifier, pubKeyHash));
        ...
        BOOST_CHECK(jsdesc_deserialized.Verify(*pzelcashParams, verifier, pubKeyHash));
    }

    {
        // Ensure that the balance equation is working.
        BOOST_CHECK_THROW(JSDescription(*pzelcashParams, pubKeyHash, rt, inputs, outputs, 10, 0), std::invalid_argument);
        BOOST_CHECK_THROW(JSDescription(*pzelcashParams, pubKeyHash, rt, inputs, outputs, 0, 10), std::invalid_argument);
    }

    {
        // Ensure that it won't verify if the root is changed.
        auto test = JSDescription(*pzelcashParams, pubKeyHash, rt, inputs, outputs, 0, 0);
        test.anchor = GetRandHash();
        BOOST_CHECK(!test.Verify(*pzelcashParams, verifier, pubKeyHash));
    }
}
