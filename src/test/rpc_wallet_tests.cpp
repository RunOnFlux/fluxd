...
BOOST_AUTO_TEST_CASE(rpc_addmultisig)
{
    ...
    BOOST_CHECK(IsValidDestination(address) && std::get_if<CScriptID>(&address) != nullptr);
...
BOOST_AUTO_TEST_CASE(rpc_wallet_z_exportwallet)
{
    ...
    libflux::SproutPaymentAddress addr = pwalletMain->GenerateNewSproutZKey();
    pwalletMain->GetSproutPaymentAddresses(addrs);
...
BOOST_AUTO_TEST_CASE(rpc_wallet_z_importwallet)
{
    ...
    auto testSpendingKey = libflux::SproutSpendingKey::random();
    ...
    BOOST_ASSERT(std::get_if<libflux::SproutPaymentAddress>(&address) != nullptr);
...
BOOST_AUTO_TEST_CASE(rpc_wallet_z_importexport)
{
    ...
    BOOST_CHECK(addrs.size()==0);
    std::set<libflux::SaplingPaymentAddress> saplingAddrs;
    ...
    BOOST_CHECK(IsValidPaymentAddress(address));
    BOOST_ASSERT(std::get_if<libflux::SproutPaymentAddress>(&address) != nullptr);
    auto newAddr = std::get<libflux::SproutPaymentAddress>(address);
...
BOOST_AUTO_TEST_CASE(rpc_wallet_encrypted_wallet_sapzkeys)
{
    ...
    // wallet should currently be empty
    std::set<libflux::SaplingPaymentAddress> addrs;
