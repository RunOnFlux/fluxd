...
bool ZelnodeBlockPayees::IsTransactionValid(const CTransaction& txNew)
{
    ...
    bool fZelnodeUpgradeActive = NetworkUpgradeActive(chainActive.Height(), Params().GetConsensus(), Consensus::UPGRADE_ZELNODES);
    ...
    int nSignaturesRequired = fZelnodeUpgradeActive ? ZNPAYMENTS_SIGNATURES_REQUIRED_AFTER_UPGRADE : ZNPAYMENTS_SIGNATURES_REQUIRED;
    ...
    if(!fZelnodeUpgradeActive || fBasicSigCountFound) {
    ...
    if(!fZelnodeUpgradeActive || fSuperSigCountFound) {
        for (CTxOut out : txNew.vout) {
    ...
    if(!fZelnodeUpgradeActive || fBAMFSigCountFound) {
        for (CTxOut out : txNew.vout) {
...
{
    bool fZelnodeUpgradeActive = NetworkUpgradeActive(nBlockHeight, Params().GetConsensus(), Consensus::UPGRADE_ZELNODES);
