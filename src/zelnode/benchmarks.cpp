...
bool GetSignedBroadcast(const ZelnodeBroadcast& znb, ZelnodeBroadcast& signedBroadcast, std::string& error)
{
    if (IsBenchmarkdRunning()) {
        ...
        if (!DecodeHexZelnodeBroadcast(signedBroadcast, response)) {
            error = "Failed to decode zelnode broadcast";
            return false;
        }

        if (!signedBroadcast.BenchmarkVerifySignature()) {
