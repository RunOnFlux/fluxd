...
void CMainSignals::ChainReorg(const CBlockIndex *pindexOldTip, const CBlockIndex *pindexNewTip, const CBlockIndex *pindexFork)
{
    auto event = [this, pindexOldTip, pindexNewTip, pindexFork] {
        m_internals->Iterate([&](CValidationInterface& callbacks) {
            callbacks.ChainReorg(pindexOldTip, pindexNewTip, pindexFork);
        });
    };
    m_internals->m_task_runner->insert(event);
}
