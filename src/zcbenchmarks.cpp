...
double benchmark_try_decrypt_notes(size_t nAddrs)
{
    ...
    auto sk = libzelcash::SproutSpendingKey::random();
    auto tx = GetValidReceive(*pzelcashParams, sk, 10, true);
...
double benchmark_increment_note_witnesses(size_t nTxs)
{
    ...
    for (int i = 0; i < nTxs; i++) {
        auto wtx = GetValidReceive(*pzelcashParams, sk, 10, true);
        auto note = GetNote(*pzelcashParams, sk, wtx, 0, 1);
...
double benchmark_create_sapling_spend()
{
    ...
    bool result = librustzelcash_sapling_spend_proof(
        ctx,
    ...
    if (!result) {
        throw JSONRPCError(RPC_INTERNAL_ERROR, "librustzelcash_sapling_spend_proof() should return true");
    }
    return t;
}
...
double benchmark_create_sapling_output()
{
    ...
    bool result = librustzelcash_sapling_output_proof(
        ctx,
    ...
    librustzelcash_sapling_proving_ctx_free(ctx);
    if (!result) {
        throw JSONRPCError(RPC_INTERNAL_ERROR, "librustzelcash_sapling_output_proof() should return true");
    }
    return t;
}
...
double benchmark_verify_sapling_spend()
{
    ...
    bool result = librustzelcash_sapling_check_spend(
                ctx,
                spend.cv.begin(),
                spend.anchor.begin(),
    ...
    if (!result) {
        throw JSONRPCError(RPC_INTERNAL_ERROR, "librustzelcash_sapling_check_spend() should return true");
    }
    return t;
}
...
double benchmark_verify_sapling_output()
{
    ...
    bool result = librustzelcash_sapling_check_output(
                ctx,
                output.cv.begin(),
                output.cm.begin(),
    ...
    librustzelcash_sapling_verification_ctx_free(ctx);
    if (!result) {
        throw JSONRPCError(RPC_INTERNAL_ERROR, "librustzelcash_sapling_check_output() should return true");
    }
    return timer_stop(tv_start);
}
