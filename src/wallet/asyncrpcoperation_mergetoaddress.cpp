...
bool AsyncRPCOperation_mergetoaddress::main_impl()
{
    ...
    if (isUsingBuilder_) {
        ...
        if (isToTaddr_) {
            ...
            auto saplingPaymentAddress = std::get_if<libflux::SaplingPaymentAddress>(&toPaymentAddress_);
    ...
    if (sproutNoteInputs_.empty() && isToZaddr_) {
        ...
        JSOutput jso = JSOutput(std::get<libflux::SproutPaymentAddress>(toPaymentAddress_), sendAmount);
        if (hexMemo.size() > 0) {
    ...
    while (!vpubNewProcessed) {
        ...
        if (jsChange > 0) {
            ...
            if (isToZaddr_ && vpubNewProcessed) {
                outputType = "target";
                jso.addr = std::get<libflux::SproutPaymentAddress>(toPaymentAddress_);
                if (!hexMemo.empty()) {
                    jso.memo = get_memo_from_hex_string(hexMemo);
                }
            }
