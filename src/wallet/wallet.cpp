...
bool CWallet::AddZKey(const libzelcash::SpendingKey &key)
{
    AssertLockHeld(cs_wallet); // mapZKeyMetadata
...
bool CWallet::AddCryptedSpendingKey(const libzelcash::PaymentAddress &address,
...
bool CWallet::LoadCryptedZKey(const libzelcash::PaymentAddress &addr, const libzelcash::ReceivingKey &rk, const std::vector<unsigned char> &vchCryptedSecret)
{
    return CCryptoKeyStore::AddCryptedSpendingKey(addr, rk, vchCryptedSecret);
}

bool CWallet::LoadZKey(const libzelcash::SpendingKey &key)
{
    return CCryptoKeyStore::AddSpendingKey(key);
}

bool CWallet::AddViewingKey(const libzelcash::ViewingKey &vk)
{
...
bool CWallet::RemoveViewingKey(const libzelcash::ViewingKey &vk)
{
...
bool CWallet::LoadViewingKey(const libzelcash::ViewingKey &vk)
{
...
void CWallet::GetFilteredNotes(std::vector<CNotePlaintextEntry> & outEntries, std::string address, int minDepth, bool ignoreSpent, bool ignoreUnspendable)
{
    ...
    libzelcash::PaymentAddress filterPaymentAddress;
    if (address.length() > 0) {
