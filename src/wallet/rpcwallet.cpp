...
UniValue getnewaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "\nResult:\n"
            "\"fluxaddress\"    (string) The new Flux address\n"
            "\nExamples:\n"
...
UniValue getaccountaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nDEPRECATED. Returns the current Flux address for receiving payments to this account.\n"
            "\nArguments:\n"
            "1. \"account\"       (string, required) MUST be set to the empty string \"\" to represent the default account. Passing any other string will result in an error.\n"
            "\nResult:\n"
            "\"fluxaddress\"   (string) The account Flux address\n"
            "\nExamples:\n"
...
UniValue getrawchangeaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "getrawchangeaddress\n"
            "\nReturns a new Flux address, for receiving change.\n"
...
UniValue setaccount(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"fluxaddress\"  (string, required) The Flux address to be associated with an account.\n"
...
UniValue getaccount(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"fluxaddress\"  (string, required) The Flux address for account lookup.\n"
            "\nResult:\n"
...
UniValue getaddressesbyaccount(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "  \"fluxaddress\"  (string) a Flux address associated with the given account\n"
            "  ,...\n"
            "]\n"
            "\nExamples:\n"
...
static void SendMoney(const CTxDestination &address, CAmount nValue, bool fSubtractFeeFromAmount, CWalletTx& wtxNew)
{
    ...
    if (!pwalletMain->CommitTransaction(wtxNew, std::ref(reservekey)))
...
UniValue sendtoaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw runtime_error(
            "sendtoaddress \"fluxaddress\" amount ( \"comment\" \"comment-to\" subtractfeefromamount )\n"
            "\nSend an amount to a given address. The amount is a real and is rounded to the nearest 0.00000001\n"
            + HelpRequiringPassphrase() +
            "\nArguments:\n"
            "1. \"fluxaddress\"  (string, required) The zelcash address to send to.\n"
            ...
            "                             The recipient will receive less Flux than you enter in the amount field.\n"
            "\nResult:\n"
...
UniValue listaddressgroupings(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp)
        throw runtime_error(
            ...
            "  [\n"
            "    [\n"
            "      \"fluxaddress\",     (string) The zelcash address\n"
...
UniValue getreceivedbyaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "getreceivedbyaddress \"fluxaddress\" ( minconf )\n"
            "\nReturns the total amount received by the given Flux address in transactions with at least minconf confirmations.\n"
            "\nArguments:\n"
            "1. \"fluxaddress\"  (string, required) The Flux address for transactions.\n"
...
UniValue sendfrom(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw runtime_error(
            "sendfrom \"fromaccount\" \"tofluxaddress\" amount ( minconf \"comment\" \"comment-to\" )\n"
            "\nDEPRECATED (use sendtoaddress). Sent an amount from an account to a Flux address.\n"
            "The amount is a real and is rounded to the nearest 0.00000001."
            + HelpRequiringPassphrase() + "\n"
            "\nArguments:\n"
            "1. \"fromaccount\"       (string, required) MUST be set to the empty string \"\" to represent the default account. Passing any other string will result in an error.\n"
            "2. \"tofluxaddress\"  (string, required) The zelcash address to send funds to.\n"
    ...
    if (strAccount.size() && !IsValidDestination(DecodeDestination(strAccount))) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid fromaccount Flux address");
    }
...
UniValue sendmany(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw runtime_error(
            ...
            "                           Those recipients will receive less Flux than you enter in their corresponding amount field.\n"
    ...
    if (!pwalletMain->CommitTransaction(wtx, std::ref(keyChange)))
...
UniValue addmultisigaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 3)
    {
        string msg = "addmultisigaddress nrequired [\"key\",...] ( \"account\" )\n"
            ...
            "2. \"keysobject\"   (string, required) A json array of Flux addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"address\"  (string) Flux address or hex-encoded public key\n"
            "       ...,\n"
            "     ]\n"
            "3. \"account\"      (string, optional) DEPRECATED. If provided, MUST be set to the empty string \"\" to represent the default account. Passing any other string will result in an error.\n"

            "\nResult:\n"
            "\"fluxaddress\"  (string) A Flux address associated with the keys.\n"
...
UniValue listtransactions(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 4)
        throw runtime_error(
            ...
            "    \"address\":\"fluxaddress\",    (string) The Flux address of the transaction. Not present for \n"
...
UniValue listsinceblock(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp)
        throw runtime_error(
            ...
            "    \"address\":\"fluxaddress\",    (string) The Flux address of the transaction. Not present for move transactions (category = move).\n"
...
UniValue gettransaction(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "      \"address\" : \"fluxaddress\",   (string) The Flux address involved in the transaction\n"
...
UniValue backupwallet(const UniValue& params, bool fHelp)
{
    ...
    std::filesystem::path exportfilepath = exportdir / clean;
...
UniValue walletpassphrase(const UniValue& params, bool fHelp)
{
    ...
    if (pwalletMain->IsCrypted() && (fHelp || params.size() != 2))
        throw runtime_error(
            ...
            "This is needed prior to performing transactions related to private keys such as sending Flux\n"
            "\nArguments:\n"
...
UniValue encryptwallet(const UniValue& params, bool fHelp)
{
    ...
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() != 1))
        throw runtime_error(
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending Flux\n"
            + HelpExampleCli("walletpassphrase", "\"my pass phrase\"") +
            "\nNow we can so something like sign\n"
            + HelpExampleCli("signmessage", "\"fluxaddress\" \"test message\"") +
...
UniValue lockunspent(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "A locked transaction output will not be chosen by automatic coin selection, when spending Flux.\n"
...
UniValue listunspent(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 3)
        throw runtime_error(
            ...
            "3. \"addresses\"    (string) A json array of Flux addresses to filter\n"
            "    [\n"
            ...
            "    \"address\" : \"address\",    (string) the Flux address\n"
    ...
    if (params.size() > 2) {
        ...
        for (size_t idx = 0; idx < inputs.size(); idx++) {
            ...
            if (!IsValidDestination(dest)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Flux address: ") + input.get_str());
            }
...
UniValue zc_sample_joinsplit(const UniValue& params, bool fHelp)
{
    ...
    JSDescription samplejoinsplit(*pfluxParams,
                                  joinSplitPubKey,
                                  anchor,
                                  {JSInput(), JSInput()},
                                  {JSOutput(), JSOutput()},
...
UniValue zc_raw_receive(const UniValue& params, bool fHelp)
{
    ...
    if (std::get_if<libflux::SproutSpendingKey>(&spendingkey) == nullptr) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Only works with Sprout spending keys");
    }
    SproutSpendingKey k = std::get<libflux::SproutSpendingKey>(spendingkey);
...
UniValue zc_raw_joinsplit(const UniValue& params, bool fHelp)
{
    ...
    for (const string& name_ : outputs.getKeys()) {
        ...
        if (std::get_if<libflux::SproutPaymentAddress>(&addrTo) == nullptr) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Only works with Sprout payment addresses");
        }
        CAmount nAmount = AmountFromValue(outputs[name_]);

        vjsout.push_back(JSOutput(std::get<libflux::SproutPaymentAddress>(addrTo), nAmount));
    }
    ...
    std::string encryptedNote2;
    {
        ...
        ss2 << jsdesc.h_sig(*pfluxParams, joinSplitPubKey);
...
UniValue z_getnewaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "\nResult:\n"
            "\"fluxaddress\"    (string) The new shielded address.\n"
            "\nExamples:\n"
...
UniValue z_listreceivedbyaddress(const UniValue& params, bool fHelp)
{
    ...
    if (std::get_if<libflux::SproutPaymentAddress>(&zaddr) != nullptr) {
    ...
    } else if (std::get_if<libflux::SaplingPaymentAddress>(&zaddr) != nullptr) {
...
UniValue z_sendmany(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            ...
            + strprintf("\nBefore Acadia (Sapling) activates, the maximum number of zaddr outputs is %d due to transaction size limits.\n", Z_SENDMANY_MAX_ZADDR_OUTPUTS_BEFORE_SAPLING)
            + HelpRequiringPassphrase() + "\n"
            "\nArguments:\n"
    ...
    if (!fromTaddr) {
        ...
        fromSapling = std::get_if<libflux::SaplingPaymentAddress>(&res) != nullptr;
    }
    ...
    for (const UniValue& o : outputs.getValues()) {
        ...
        if (!IsValidDestination(taddr)) {
            ...
            if (IsValidPaymentAddress(res)) {
                ...
                bool toSapling = std::get_if<libflux::SaplingPaymentAddress>(&res) != nullptr;
...
UniValue z_mergetoaddress(const UniValue& params, bool fHelp)
{
    ...
    if (!fEnableMergeToAddress) {
        throw JSONRPCError(RPC_WALLET_ERROR, "Error: z_mergetoaddress is disabled. Run './flux-cli help z_mergetoaddress' for instructions on how to enable this feature.");
    }
    ...
    if (!IsValidDestination(taddr)) {
        ...
        if (IsValidPaymentAddress(decodeAddr)) {
            if (std::get_if<libflux::SaplingPaymentAddress>(&decodeAddr) != nullptr) {
    ...
    if (!sproutNoteInputs.empty() || !saplingNoteInputs.empty() || !IsValidDestination(taddr)) {
        ...
        // therefore we cannot create transactions before Acadia (Sapling) activates.
        if (!saplingActive) {
            throw JSONRPCError(
                RPC_INVALID_PARAMETER, "Cannot create shielded transactions before Acadia (Sapling) has activated");
        }
