...
CWalletTx GetValidReceive(const libzelcash::SpendingKey& sk, CAmount value, bool randomInputs) {
    return GetValidReceive(*params, sk, value, randomInputs);
}

libzelcash::Note GetNote(const libzelcash::SpendingKey& sk,
                       const CTransaction& tx, size_t js, size_t n) {
    return GetNote(*params, sk, tx, js, n);
}

CWalletTx GetValidSpend(const libzelcash::SpendingKey& sk,
...
TEST(wallet_tests, FindMyNotes) {
    ...
    auto sk = libzelcash::SpendingKey::random();
    auto sk2 = libzelcash::SpendingKey::random();
