...
Note that fluxd will need to be stopped and restarted for edits in the config file to take effect. 

### Using `backupwallet`

To create a backup of your wallet, use:

`flux-cli backupwallet <nameofbackup>`.
...
If you generate a new Flux address, it will not be reflected in the backup file.
...
`flux-cli z_exportwallet <nameofbackup>`
...
`flux-cli z_importwallet </path/to/exportdir/nameofbackup>`
...
`flux-cli z_exportkey <z-address>`
...
`flux-cli dumpprivkey <t-address>`

This will return the private key and will not create a new file.

To import a private key for a shielded address, use:

`flux-cli z_importkey <z-priv-key>`
