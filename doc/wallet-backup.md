...
Backing up your Zelcash private keys is the best way to be proactive about preventing loss of access to your ZEC.
...
These instructions are specific for the officially supported Zelcash Linux client. For backing up with third-party wallets, please consult with user guides or support channels provided for those services.
...
For all methods, you will need to include an export directory setting in your config file (`zelcash.conf` located in the data directory which is `~/.zelcash/` unless it's been overridden with `datadir=` setting):
...
Note that zelcashd will need to be stopped and restarted for edits in the config file to take effect. 

### Using `backupwallet`

To create a backup of your wallet, use:

`zelcash-cli backupwallet <nameofbackup>`.
...
If you generate a new Zelcash address, it will not be reflected in the backup file.
...
`zelcash-cli z_exportwallet <nameofbackup>`
...
`zelcash-cli z_importwallet </path/to/exportdir/nameofbackup>`
...
`zelcash-cli z_exportkey <z-address>`
...
`zelcash-cli dumpprivkey <t-address>`

This will return the private key and will not create a new file.

To import a private key for a shielded address, use:

`zelcash-cli z_importkey <z-priv-key>`
...
`zelcash-cli help importprivkey`
