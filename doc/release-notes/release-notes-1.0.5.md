...
Jack Grigg (22):
      ...
      Instruct users to run zelcash-fetch-params if network params aren't available
      ...
      Throw an error if zelcash.conf is missing
      Show a friendly message explaining why zelcashd needs a zelcash.conf
      Fix gtest ordering broken by #1949
      Debian package lint
      Generate Debian control file to fix shlibs lint
      Create empty zelcash.conf during performance measurements
      Create empty zelcash.conf during coverage checks
...
Simon Liu (13):
      ...
      Remove UTF-8 BOM efbbbf from zelcash.conf to avoid problems with command line tools
      Closes #1097 so zelcash-cli now displays license info like zelcashd.
